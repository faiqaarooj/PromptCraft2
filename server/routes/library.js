/**
 * /api/library  –  read-only prompt template library
 *
 *  GET /api/library          – list all templates
 *  GET /api/library/:id      – get a single template
 *  GET /api/library?category=<cat>  – filter by category
 *  GET /api/library?tool=<tool>     – filter by tool
 */
...
router.get("/:id", (req, res) => {
  const template = LIBRARY.find((t) => t.id === req.params.id);
